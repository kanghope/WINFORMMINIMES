...
namespace MiniMES.Infastructure.Services
{
    ...
    public class SerialDeviceService : IDisposable
    {
        ...
        public void Open(string portName, int baudRate = 9600)
        {
            /*
             옵션명,설정값,의미
            PortName,"""COM1""",연결될 통로 이름
            BaudRate,9600,통신 속도 (초당 9600비트)
            DataBits,8,실제 데이터 알맹이의 크기
            Parity,None,별도의 오류 검사 비트 사용 안 함
            StopBits,One,끝났다는 신호로 1비트 사용
             */
            try
            {
                // 재연결 타이머를 위해 마지막 정보 저장
        ...
        private async Task CheckConnection()
        {
            ...
            if (_port == null || !_port.IsOpen)
            {
                ...
                catch(Exception ex)
                {
                    ...
                    // 1. 연결 상태 업데이트 (여전히 끊김 상태임을 명시)
                    OnConnectionStatusChanged?.Invoke(false);

                    // 2. 구체적인 에러 로그 남기기 (디버깅용)
                    // 장치가 이미 사용 중(UnauthorizedAccessException)인지, 포트가 없는지(IOException) 등을 기록
                    Console.WriteLine($"[DEBUG] 재연결 시도 실패: {ex.Message}");

                    // 3. (선택사항) 통신 로그 이벤트가 있다면 화면에 출력
                    // OnLogReceived?.Invoke($"재연결 실패: {ex.Message}");

                    // 4. 리소스 정리
                    // Open 시도 중에 객체가 꼬였을 수 있으므로 안전하게 Close 호출
        ...
        private async void ProcessProduction(string eqCode, string rawData, string okQty, string ngQty)
        {
            ...
            try
            {
                ...
                string currentUserId = !string.IsNullOrEmpty(UserSession.UserId)
                                       ? UserSession.UserId
                                       : "SYSTEM";
